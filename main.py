...
def periodic_report(start_time, end_time,
    ...
    print("WEBHOOK REPORT SENT AFTER {0} SECONDS".format(str(seconds)))
    try:
        embed=discord.Embed(title="Miner Uptime {0}".format(uptime), color=0xffffff)
        embed.set_author(name="Duinocoin Anyfiddle Miner")
        embed.add_field(name=str(get_prefix("H/s", hashrate, 2)), value="hashrate", inline=True)
        embed.add_field(name="{0} shares/s".format(str(round(shares/seconds, 1))), value="Mined {0} Shares".format(str(shares)), inline=True)
        embed.add_field(name=str(int(hashrate*seconds)), value="hashes solved", inline=True)
        embed.set_footer(text="Statistics for the last {0} seconds".format(seconds))
...
if __name__ == "__main__":
    ...
    print("Reporting every", user_settings['report_sec'], "seconds")
    Miner.greeting()
